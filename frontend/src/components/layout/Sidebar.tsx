...
import { LayoutDashboard, ClipboardList, Settings, ScanLine, Cloud } from "lucide-react";
import { EdgeModeControl } from "@/components/EdgeModeControl";
...
const sections: Section[] = [
  ...
  },
  {
    title: "Edge",
    items: [
      { label: "Edge receiving · live", icon: ScanLine, view: { kind: "edge-live" } },
      { label: "Control tower · cloud", icon: Cloud, view: { kind: "cloud-tower" } },
    ],
  },
  {
    title: "System",
...
function isItemActive(item: NavItem, view: View): boolean {
  ...
  if (item.view.kind === "edge-live") return view.kind === "edge-live";
  if (item.view.kind === "cloud-tower") return view.kind === "cloud-tower";
  return false;
}
...
export function Sidebar() {
  ...
  return (
    <aside className="flex flex-col w-[240px] shrink-0 h-screen bg-white border-r border-divider sticky top-0">
      ...
      {/* Global runtime-mode switch — device-level, affects every console */}
      <EdgeModeControl />
